...
本地离线 `mermaid .mmd` 查看 + 评审工具：服务端以仓库根为静态根托管 viewer，前端在图上点节点/连线打标评论，评论持久化到 SQLite。纯标准库 + 原生 JS + 本地 vendor 库，不联网。仅绑 loopback。
...
| `main` | fn | viewer_server.py | 入口：绑 loopback → serve_forever |
...
# 访问
#   查看器: http://127.0.0.1:8123/apps/mermaid-viewer/viewer.html
